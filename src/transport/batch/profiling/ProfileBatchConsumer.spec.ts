...
vi.mock('node:zlib', () => ({
  default: {
    deflate: vi.fn((_buf: Buffer, cb: (err: null, result: Buffer) => void) => cb(null, Buffer.from('compressed'))),
  },
}));
...
describe('ProfileBatchConsumer', () => {
  ...
  describe('upload()', () => {
    ...
    it('posts to the intake URL with DD-API-KEY and User-Agent headers', async () => {
      ...
      expect(call[0]).toBe(INTAKE_URL);
      const headers = call[1]?.headers as Record<string, string>;
      expect(headers['DD-API-KEY']).toBe(CLIENT_TOKEN);
    ...
    it('skips and deletes files with fewer than 2 lines', async () => {
      fsMocks.readdir.mockResolvedValue(['profile.log']);
