 ...
 * hooks that tag every other main-process RUM event and span with the execution context active at
 * that event's timestamp, backed by a disk-persisted history so a crash file replayed from a
 * previous run still resolves to the context that was active when the crash happened.
 */
