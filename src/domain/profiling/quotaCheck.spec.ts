  ...
  describe('URL construction', () => {
    it('builds the correct URL for datadoghq.com', async () => {
      ...
      expect(vi.mocked(fetch).mock.calls[0][0]).toBe(
        'https://quota.browser-intake-datadoghq.com/api/v2/profiling/quota?session_id=session-abc&ddsource=electron'
      );
    });
    ...
    it('builds the correct URL for datadoghq.eu', async () => {
      ...
      await promise;
      expect(vi.mocked(fetch).mock.calls[0][0]).toBe(
        'https://quota.browser-intake-datadoghq.eu/api/v2/profiling/quota?session_id=session-abc&ddsource=electron'
      );
    });
    ...
    it('builds the correct URL for us3.datadoghq.com', async () => {
      ...
      await promise;
      expect(vi.mocked(fetch).mock.calls[0][0]).toBe(
        'https://quota.browser-intake-us3-datadoghq.com/api/v2/profiling/quota?session_id=session-abc&ddsource=electron'
      );
    });
    ...
    it('builds the correct URL for datad0g.com (internal staging)', async () => {
      ...
      await promise;
      expect(vi.mocked(fetch).mock.calls[0][0]).toBe(
        'https://quota.browser-intake-datad0g.com/api/v2/profiling/quota?session_id=session-abc&ddsource=electron'
      );
    });
    ...
    it('routes through proxy with ddforwardSubdomain=quota', async () => {
      ...
      await promise;
      expect(vi.mocked(fetch).mock.calls[0][0]).toBe(
        'http://proxy.example.com?ddforward=%2Fapi%2Fv2%2Fprofiling%2Fquota%3Fsession_id%3Dsession-abc%26ddsource%3Delectron&ddforwardSubdomain=quota'
      );
    });
