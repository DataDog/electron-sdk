...
test('tags main-process spans with _dd.execution_context.id, like _dd.session.id and _dd.view.id', async ({
  mainPage,
  intake,
}) => {
  await mainPage.flushTransport();
  const mainEvent = (await intake.getEventsByType('execution_context')).find(
    (e) => asExecutionContextEvent(e.body).execution_context.type === 'main-process'
  )!;
  const executionContextId = asExecutionContextEvent(mainEvent.body).execution_context.id;

  await mainPage.mainPing();
  await mainPage.flushTransport();

  const span = await intake.waitForSpan((s) => s.name === 'electron.main.handle' && s.resource === 'ping');
  expect(span.meta['_dd.execution_context.id']).toBe(executionContextId);
});
...
test('a renderer that crashes and reloads the same window gets a fresh execution context', async ({
  mainPage,
  intake,
}) => {
  await mainPage.flushTransport();
  ...
  // flush, and there's no event to synchronously await for "the reload's navigation has now
  // started", so flush repeatedly until the revival's events actually show up instead of flushing
  ...
  while (afterRevival.length < targetCount && Date.now() < deadline) {
    await mainPage.flushTransport();
    afterRevival = await intake.getEventsByType('execution_context');
  }
  ...
  const revivedEnd = revivedEvents.find(
    (e) => asExecutionContextEvent(e.body).execution_context.id === firstBody.execution_context.id
  );
  expect(revivedEnd).toBeDefined();

  const revivedStart = revivedEvents.find(
    (e) => asExecutionContextEvent(e.body).execution_context.id !== firstBody.execution_context.id
  );
  expect(revivedStart).toBeDefined();
...
test.describe('session renewal', () => {
  ...
  test('the action that triggers a session renewal is tagged with the new execution_context, not the old one', async ({
    ...
    // has completed, is a real renderer-sourced action tagged via RendererProcessContexts's own
