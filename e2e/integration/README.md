...
This loads the SDK's instrumentation and injects the SDK's preload script into every renderer
process via `patchBrowserWindow`. The regular SDK `init()` initializes dd-trace-electron afterward.
