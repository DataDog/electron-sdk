...
Some files in this repo are produced by tooling and overwritten on the next run. Do not edit them by hand:
...
| `LICENSE-3rdparty.csv` | hand-curated, but validated by `node scripts/check-licenses.ts` | See "Adding Dependencies" |
