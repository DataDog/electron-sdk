...
The SDK injects a preload script (`@datadog/electron-sdk/electron/preload`) into every renderer process. The preload exposes `DatadogEventBridge` via `contextBridge`, enabling the Browser SDK to route RUM events through IPC to the Electron SDK instead of sending them directly to Datadog.

`patchBrowserWindow` registers the preload via two complementary mechanisms:

- `session.defaultSession.registerPreloadScript()`: primary, covers all windows on the default session.
- `BrowserWindow` subclassing: covers windows that use a non-default session.
